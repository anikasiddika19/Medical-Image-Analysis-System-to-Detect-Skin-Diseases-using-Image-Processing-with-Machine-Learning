function varargout = dermatitis(varargin)
% DERMATITIS MATLAB code for dermatitis.fig
%      DERMATITIS, by itself, creates a new DERMATITIS or raises the existing
%      singleton*.
%
%      H = DERMATITIS returns the handle to a new DERMATITIS or the handle to
%      the existing singleton*.
%
%      DERMATITIS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DERMATITIS.M with the given input arguments.
%
%      DERMATITIS('Property','Value',...) creates a new DERMATITIS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before dermatitis_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to dermatitis_OpeningFcn via varargin.
% 
...
% Edit the above text to modify the response to help dermatitis

% Last Modified by GUIDE v2.5 31-Dec-2019 22:18:57

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @dermatitis_OpeningFcn, ...
                   'gui_OutputFcn',  @dermatitis_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before dermatitis is made visible.
function dermatitis_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to dermatitis (see VARARGIN)

% Choose default command line output for dermatitis
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes dermatitis wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = dermatitis_OutputFcn(hObject, eventdata, handles) 
